...
def upgrade() -> None:
    ...
    if "scheduler_job_runs" not in tables:
        op.create_table(
            "scheduler_job_runs",
        ...
        existing = {i["name"] for i in inspector.get_indexes("scheduler_job_runs")}
        for idx_name, cols in [
            ("ix_scheduler_job_runs_org_id", ["organization_id"]),
            ("ix_scheduler_job_runs_job_name", ["job_name"]),
            ("ix_scheduler_job_runs_status", ["status"]),
            ("ix_scheduler_job_runs_started_at", ["started_at"]),
            ("ix_scheduler_job_runs_finished_at", ["finished_at"]),
            ("ix_scheduler_job_runs_org_job_started", ["organization_id", "job_name", "started_at"]),
        ]:
            if idx_name not in existing:
                op.create_index(idx_name, "scheduler_job_runs", cols)
