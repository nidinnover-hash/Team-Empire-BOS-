...
async def _bulk_employee_perf(
    ...
    *,
    ...
    emp_filter = [Employee.organization_id == org_id, Employee.is_active == True]  # noqa: E712
        ...
        (
            await db.execute(
                select(func.count(Employee.id)).where(
                    Employee.organization_id == org_id, Employee.is_active == True  # noqa: E712
                )
            )
        ).scalar_one() or 0
    )
        ...
        (
            await db.execute(
                select(func.count(Department.id)).where(
                    Department.organization_id == org_id, Department.is_active == True  # noqa: E712
                )
            )
        ).scalar_one() or 0
    )
