...
async def _bulk_employee_perf(
    ...
    *,
    ...
    emp_filter = [Employee.organization_id == org_id, Employee.is_active.is_(True)]
...
async def get_org_performance(
    ...
    dept_agg_query = (
        ...
        .outerjoin(Employee, (Employee.department_id == Department.id) & (Employee.is_active.is_(True)))
        .outerjoin(
            EmployeeWorkPattern,
            (EmployeeWorkPattern.employee_id == Employee.id)
            & (EmployeeWorkPattern.organization_id == org_id)
