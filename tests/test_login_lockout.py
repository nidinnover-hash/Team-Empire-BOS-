...
from app.core.middleware import LOGIN_FAIL_MAX, _login_failures
...
async def test_lockout_after_max_failures_on_token_endpoint(client):
    ...
    for _ in range(LOGIN_FAIL_MAX):
        r = await client.post("/token", data={"username": "bad@ai.com", "password": "wrongpass1"})
        assert r.status_code == 401
...
async def test_lockout_message_is_descriptive(client):
    for _ in range(LOGIN_FAIL_MAX):
        await client.post("/token", data={"username": "bad@ai.com", "password": "wrongpass1"})
...
async def test_lockout_after_max_failures_on_web_login(client):
    for _ in range(LOGIN_FAIL_MAX):
        r = await client.post("/web/login", data={"username": "bad@ai.com", "password": "wrongpass1"})
        assert r.status_code == 401
...
async def test_lockout_after_max_failures_on_auth_router(client):
    for _ in range(LOGIN_FAIL_MAX):
        r = await client.post(
            "/api/v1/auth/login",
            data={"username": "bad@ai.com", "password": "wrongpass1"},
        )
        assert r.status_code == 401
...
async def test_cross_endpoint_failures_share_bucket(client):
    ...
    for _ in range(LOGIN_FAIL_MAX - half):
        await client.post("/web/login", data={"username": "bad@ai.com", "password": "wrongpass1"})
